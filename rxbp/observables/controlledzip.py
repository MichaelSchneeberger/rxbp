...
def controlled_zip(left: ObservableBase, right: ObservableBase,
    ...
    def unsafe_subscribe(scheduler: Scheduler, subscribe_scheduler: Scheduler):
        ...
        def start_zipping(left_val: Any,
            ...
            if not has_left_elem and not has_right_elem:
                ...
                result_ack = zip_out_ack.merge_ack(left_out_ack).merge_ack(right_out_ack)
                if isinstance(is_sync, SynchronousLeft):
                    result_ack.connect_ack(is_sync.right_in_ack)
                elif isinstance(is_sync, SynchronousRight):
                    result_ack.connect_ack(is_sync.left_in_ack)
                else:
    ...
    class ControlledZippedObservable(ObservableBase):
