...
class ActiveStateMixin(FlatMapState):
    ...
    is_outer_completed: bool
    # outer_certificate: ContinuationCertificate
...
@dataclass(frozen=True, slots=True)
class SubscribedStateOuterCompleted(StopContinuationStateMixin, AwaitUpstreamStateMixin):
    """Await first upstream item"""
...
@dataclass(frozen=True, slots=True)
class AwaitDownstreamOuterCompletedState(StopContinuationStateMixin, AwaitDownstreamStateMixin):
    """Await downstream request"""
...
@dataclass(frozen=True, slots=True)
class TerminatedStateMixin(FlatMapState):
