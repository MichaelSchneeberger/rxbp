...
class TestWindow(TestCase):
    ...
    def test_window_4(self):
        ...
        def action2(scheduler, state=None):
            counter = [0]
            def subscribe_to_next_observer(v):
                v[1].subscribe(results[counter[0]])
                counter[0] += 1

            parent_results.subscribe(on_next=subscribe_to_next_observer)
            subscription[0] = s[0].subscribe(observer=parent_results)
        scheduler.schedule_absolute(200, action2)
        scheduler.start()
        ...
        results2.bp_messages.assert_equal(
            bp_response(420, 1),
            bp_response(430, 1),
            bp_response(440, 1),
            bp_response(450, 0),
